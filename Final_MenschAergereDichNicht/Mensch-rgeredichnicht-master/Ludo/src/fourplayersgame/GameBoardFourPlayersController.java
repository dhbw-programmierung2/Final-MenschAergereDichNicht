...
public class GameBoardFourPlayersController {
	...
	public void circleOnActionM(MouseEvent event) {
		String tempCircleID = event.getPickResult().getIntersectedNode().getId();
		//Zuordnung zur Logik
				Runnable r = new Runnable(); //Das hier ist die Logik
		
		
		
			for(int index = 0 ;index<fieldsGui.size();index++ ) {
				//Image[] imagesOnField = r.getFIeld();
				//imagesOnField[]= fieldsGui.get(index);
					
				if (fieldsGui.get(index).equals(tempCircleID)) {	
					//r.move2(spieler, kegel, wuerfelzahl);
...
//		}
	...
	public void buttonRollTheDiceOnAction(ActionEvent event) {
		...
		if(numberButtonPressed==LauncherController.numberOfPlayers) {
